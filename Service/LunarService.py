...
'''
这个类主要记录如何从阳历生日转换成生辰八字的
'''

def lunarday(chinese_year,chinese_month,chinese_day,english_time,runyue:bool):
    '''
    最好输入的是阳历生日防止阴历有闰月的问题
    :param chinese_year:
    :param chinese_month:
    :param chinese_day:
    :param english_time:
    :return:
    '''
    listtime = change(chinese_year,chinese_month,chinese_day)
    if listtime is not None:
        if not runyue:
            return day(int(listtime[0][0]), int(listtime[0][1]), int(listtime[0][2]), english_time)
        else:
            return day(int(listtime[1][0]), int(listtime[1][1]), int(listtime[1][2]), english_time)
    else:
        return None

def day(english_year, english_month, english_day, english_time):
    # 旧历的计算
    '''

    :param english_year:  阳历出生的年月日
    :param english_month:
    :param english_day:
    :param english_time: 时辰
    :return:  [5] 生辰八字，[4] 年分；【3】阴历生日 【2】 日柱的天干  【1】 属相 【0】 年月日的天干地支
    '''
    ...
    tianganOfDaySymbol = tiangans.index(f5) + 1
    ganzhiOfTime = calculateTime(tianganOfDaySymbol, english_time)
    f6=f3+"-"+ganzhiOfTime
    return [f3, f4, f5, f2, english_year,f6]
